...
void fork18()//test if the shell will send ctrl+c to every  process in forground process group?
{
    pid_t pid[N];
    int i;
    ccount = N;
...
void fork19()//test if the shell will send ctrl+c to every  process in forground process group?
{
    pid_t pid[N];
    int i;
    ccount = N;
    ...
    for (i = 0; i < N; i++)
	if ((pid[i] = fork()) == 0) {
		if ((rc = setpgid(0, 0)) < 0)//子进程将自己放到一个新的进程组中（进程组ID与子进程的PID相同)
